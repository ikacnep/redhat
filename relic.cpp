...
bool IsRelic(const CItem& item) {
    ...
    // TODO: remove ID-specific checks and replace with just relic IDs after migration.
    if (item.Id == ascension_crown.Id || item.Id == ascension_staff.Id) {
        return true;
    }

    // Used to have Golden Wrath bijou on the maps. Need to check a unique relic property.
    if (item.Id == 17730 || item.Id == 17473) {
        for (const auto& effect : item.Effects) {
            if (effect.Id1 == stats::protection_astral && effect.Value1 == 2) {
                return true;
            }
        }
        return false;
    ...
    // Used to have Vampic bijou on the maps. Need to check a unique relic property.
    if (item.Id == 54369 || item.Id == 54626) {
        for (const auto& effect : item.Effects) {
            if (effect.Id1 == stats::speed && effect.Value1 == 1) {
                return true;
            }
        }
        return false;
...
bool MigrateRelics(bool dry_run) {
    ...
    if (!query) {
        Printf(LOG_Error, "[relic] Failed to query characters for sigil migration: %s\n", SQL_Error().c_str());
        return false;
    }

    std::unordered_map<uint16_t, uint16_t> rewrite_old_ids = {
        {54369, 37985}, // Vampic Ring -> Shaman Ring
        {54626, 38242}, // Vampic Amulet -> Shaman Amulet
        {17473, 54465}, // Golden Wrath Ring -> Crimson Red Crystal Ring
        {17730, 54722}, // Golden Wrath Amulet -> Crimson Red Crystal Amulet
    };

    // First, load all characters and bestow sigils where necessary.
    std::vector<CCharacter> all_characters;
    std::unordered_map<int, uint32_t> id_to_sigil;
    std::unordered_map<int, std::unordered_map<uint16_t, int>> id_to_available_relics;
    std::unordered_map<int, std::vector<int>> login_id_to_character_id;
    ...
    for (int i = 0; i < character_rows; i++) {
        ...
        login_id_to_character_id[chr.LoginID].push_back(chr.ID);
        ...
        for (auto& relic_id: relic_id_to_sex) {
            ...
            if (amount > 0) {
                id_to_available_relics[chr.ID][relic_id.first] += amount;
            }
        }

        if (!id_to_available_relics[chr.ID].empty()) {
            std::string claimed_relics_str;
            for (auto& [relic_id, amount] : id_to_available_relics[chr.ID]) {
        ...
        if (dry_run) {
            id_to_sigil[chr.ID] = id_to_sigil.size() + 1;
            Printf(LOG_Info, "[relic] dry-run: would bestow a sigil upon character %d. Using placeholder %u\n", chr.ID, id_to_sigil[chr.ID]);
            continue;
        }

        uint32_t sigil = BestowSigil(chr);
        if (!sigil) {
            Printf(LOG_Warning, "[relic] Failed to bestow a sigil upon character %d\n", chr.ID);
            continue;
        }

        Printf(LOG_Info, "[relic] Bestowed a sigil %u upon character %d\n", sigil, chr.ID);
        id_to_sigil[chr.ID] = sigil;
    }

    int available = 0;
    for (auto& [character_id, relics] : id_to_available_relics) {
        for (auto& [relic_id, amount] : relics) {
            available += amount;
        }
    }

    Printf(LOG_Info, "[relic] There are %d characters that should have sigils, and %d available relics to emboss\n", (int)id_to_sigil.size(), available);

    // Second, find all relics on characters and emboss them with the appropriate sigils.
    for (auto& chr : all_characters) {
        if (id_to_sigil.count(chr.ID) == 0) {
            continue;
        }
        ...
        if (dry_run) {
            Printf(LOG_Info, "[relic] Dry-run: not saving embossed relics for character %d\n", chr.ID);
            continue;
        }
        ...
        if (affected_rows != 1) {
            Printf(LOG_Warning, "[relic] Update affected %d rows when saving embossed relics for character %d\n", affected_rows, chr.ID);
        }
    }

    // Recount remaining available relics.
    int remaining = 0;
    for (auto& [character_id, relics] : id_to_available_relics) {
        for (auto& [relic_id, amount] : relics) {
            remaining += amount;
    ...
    Printf(LOG_Info, "[relic] After embossing bags and dresses, there are %d relics available to emboss\n", remaining);

    // Third, find all relics shelved and emboss them with the appropriate sigils.
    // Need to check all servers since we're also rewriting old relic IDs.
    SimpleSQL shelf("SELECT login_id, server_id, cabinet, items FROM shelf ORDER BY login_id ASC, server_id ASC, cabinet ASC;");
    if (!shelf) {
        Printf(LOG_Error, "[relic] Failed to query shelved items for sigil migration: %s\n", SQL_Error().c_str());
        return false;
    }

    int shelf_rows = SQL_NumRows(shelf.result);

    for (int i = 0; i < shelf_rows; i++) {
        auto row = SQL_FetchRow(shelf.result);
        ...
        for (auto& item : items.Items) {
            ...
            std::vector<int> can_belong_to_character_ids;
            for (auto& character_id : login_id_to_character_id[login_id]) {
                if (id_to_available_relics[character_id][item.Id] > 0) {
                    can_belong_to_character_ids.push_back(character_id);
                }
            }

            if (can_belong_to_character_ids.size() != 1) {
                std::string available_characters;
                for (auto& character_id : can_belong_to_character_ids) {
                    if (!available_characters.empty()) {
                        available_characters += ", ";
                    }
                    available_characters += std::to_string(character_id);
                }
                if (can_belong_to_character_ids.empty()) {
                    available_characters = "none";
                }
                Printf(LOG_Warning, "[relic] Shelved relic (item ID %u) can belong to %d characters for login ID %d, server ID %d, cabinet %d, skipping embossing. Available characters: %s\n", item.Id, (int)can_belong_to_character_ids.size(), login_id, server_id, cabinet, available_characters.c_str());
                continue;
            }

            auto character_id = can_belong_to_character_ids[0];
            uint32_t sigil = id_to_sigil[character_id];
            id_to_available_relics[character_id][item.Id]--;

            Printf(LOG_Info, "[relic] Embossing shelved relic (item ID %u) for character %d with sigil %u\n", item.Id, character_id, sigil);
            EmbossSigil(item, sigil);
            updated = true;
        }
        ...
        if (dry_run) {
            Printf(LOG_Info, "[relic] Dry-run: not saving embossed shelved relics for login ID %d, server ID %d, cabinet %d\n", login_id, server_id, cabinet);
            continue;
        }
        ...
        int affected_rows = SQL_AffectedRows();
        if (affected_rows != 1) {
            Printf(LOG_Warning, "[relic] Update affected %d rows when saving embossed shelved relics for login ID %d, server ID %d, cabinet %d\n", affected_rows, login_id, server_id, cabinet);
        }
    }
